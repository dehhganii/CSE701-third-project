 ...
 */
void Graphic::draw_point(const Color& color, int x, int y)
{
    set_color(color, x, y);
...
/**
 * @brief Method to get two point and draw a line between them. It is important that the first point
 * must have lower x
 * 
 ...
 */
void Graphic::draw_line(const Color& color, int x1, int y1, int x2, int y2, double  m)
{

    for(int i=x1; i < x2; i++)
    {
        int64_t temp = (int64_t) roundtol((m * i) + (y1- m * x1));
